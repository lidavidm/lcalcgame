class Concept {
    identifier() { return 'Concept'; }
    parameters() { return []; }
    toString() {
        let arr = ['a', 'b', 'c', 'd', 'e'];
        return this.identifier() + '(' + this.parameters().reduce((p,c,i) => p + arr[i] + ', ', '') + ')';
    }
}
...
var concepts = [
    ...
    */
    'Map( Identity, E:Collection ) -> E',
    'Map( Lambda, Collection ) -> Collection',

    // Place in a hole
    // * Not sure about these!!
    'Place( HoleyExpression, _, Any ) -> Any',
    ...
    // Apply rules
    'Apply( Lambda(T:), Any ) -> T',
    'Identity := Apply( Lambda(x), E:Any ) -> E',
    'Dup := Apply( Lambda(xx), E:Any ) -> E E', // Output can be a set of expressions; however len>1 forbids 'Dup' rule to be embedded (it must be top-level).
    ...
    // Conditional rules
    'Cond( _b, Any, Any )',
    'Cond( True, B:Any, Any ) -> B',
    'Cond( False, Any, E:Any ) -> E',
    ...
    // Equal rule
    {
        ...
        reverse: (bool) => { // An inverse of the 'forward' function, so that we can backchain:
            if (bool === 'True') {
                return [ 'X', 'X' ];
            } else {
                return [ 'X', 'Y' ];
