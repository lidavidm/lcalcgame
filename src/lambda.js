...
var LambdaCalculus = (() => {
    ...
    var color_map = {};
    var nextColor = () => { return '#'+Math.floor(Math.random()*16777215).toString(16); };
    var genLambda = (name) => {
     ...
     */
    pub.parse = (lcalc_expr) => {

        return parse_recur(lcalc_expr);

    };
    var parse_recur = (lcalc_expr) => {

        if (lcalc_expr.length === 0) return null;

        var exprs = [];
        for (let e = 0; e < lcalc_expr.length; e++) {
            ...
            if (typeof lcalc_expr[e] === 'string' && lcalc_expr[e].indexOf('λ') > -1) {
                ...
                for(let i = e + 1; i < lcalc_expr.length; i++) {
                ...
                exprs.push(lambda);
                break;
            } else if (typeof lcalc_expr[e] === 'string') { // var name
                var var_expr = genVar(lcalc_expr[e]);
                exprs.push(var_expr);
            } else if (typeof lcalc_expr[e] === 'object') { // array (parentheses)
                var paren_expr = genParen();
                var inside = parse_recur(lcalc_expr[e]);
                inside.forEach((exp) => paren_expr.addArg(exp));
        ...
        if (exprs.length > 1) {
            var paren = genParen();
            exprs.forEach((exp) => paren.addArg(exp));
            console.log(paren);
            return paren;
        }
        else return exprs[0];
