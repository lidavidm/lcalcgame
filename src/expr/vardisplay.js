...
class DisplayChest extends Expression {
    ...
    drawInternal(ctx, pos, boundingSize) {
        ...
        ctx.draw(ChestImages.lidOpen(this.name), pos.x + offsetX, pos.y, size.w * scale.x, size.h * scale.y);
        this.holes[0].pos = {
            x: this.childPos.x,
            y: this.childPos.y,
        };
    ...
    drawInternalAfterChildren(ctx, pos, boundingSize) {
        ...
        let scale = this.absoluteScale;
        let offsetX = (boundingSize.w - size.w * scale.x) / 2;
        ctx.draw(ChestImages.base(this.name), pos.x + offsetX, pos.y, size.w * scale.x, size.h * scale.y);
    }
}
