...
function constructGanttChart(table, chapters) {
    ...
    var row_names = [ level_name_cell ]; // we'll take the transpose of this
    ...
    const toCell = (level, concept) {
        if (levelContainsConcept(level, concept))
            return { 'text':'', 'color':concept.color };
        else
            return { 'text':'', 'color':rbg(255, 255, 255) };
    };
    ...
    for (chapter of chapters) {
        ...
        for (level of levels) {
            for (row of rows) {
                ...
                row.push( toCell(level, concept) );
            } // for each row, # of cells == # of levels + 1
        }
    }

    // Create DOM table from data
    const setupDOMCell = (DOMcell, cell_desc) => {
        $(DOMcell).css('background-color', rgbToCSS('color' in cell_desc ? cell_desc.color : rgb(255, 255, 255) ));
        $(DOMcell).innerHTML = 'text' in cell_desc ? cell_desc.text : ' ';
    };
